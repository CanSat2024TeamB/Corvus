...
async def main():
    ...
    coords = [target for pair in zip([target_coordinates_1] * num, [target_coordinates_2] * num) for target in pair] + [target_coordinates_1]


    await drone.add_sequence_task(sequence_test_mission(drone, speed, *coords))
    try:
